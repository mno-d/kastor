...
function windowsBashOptions(): BashToolOptions | undefined {
  ...
  return {
    spawnHook: (spawnContext) => ({
      ...
      env: {
        ...spawnContext.env,
        LANG: spawnContext.env.LANG ?? "C.UTF-8",
        LC_ALL: spawnContext.env.LC_ALL ?? "C.UTF-8",
        PYTHONUTF8: spawnContext.env.PYTHONUTF8 ?? "1",
        PYTHONIOENCODING: spawnContext.env.PYTHONIOENCODING ?? "utf-8",
        // Git Bash/MSYS rewrites arguments like /F into F:/ by default.
        // That breaks Windows tools such as taskkill during process cleanup.
        MSYS2_ARG_CONV_EXCL: spawnContext.env.MSYS2_ARG_CONV_EXCL ?? "*",
      },
    }),
  };
}
...
export async function runShellTool(input: BashToolInput, context: ToolContext): Promise<ToolResponse> {
  const tool = createBashTool(context.cwd, windowsBashOptions());
  const timeout = input.timeout === undefined ? 20 : Math.min(input.timeout, 60);
