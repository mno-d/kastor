...
Kastor exposes local coding capabilities over MCP. Treat it as remote access
...
Kastor only opens workspaces under configured roots.
...
Broad roots are for private machines only. Public examples, templates, and
shared installs should never default to whole-PC access.
...
Kastor needs `KASTOR_PUBLIC_BASE_URL` so MCP clients can discover OAuth
...
Do not include `/mcp` in `KASTOR_PUBLIC_BASE_URL`.

By default, Kastor derives allowed Host headers from the local host and public
URL. Use `KASTOR_ALLOWED_HOSTS=*` only for intentional local debugging.

## Tunnels

Kastor does not manage tunnels. Your tunnel or reverse proxy should point to:
...
protection in front of public tunnels. Kastor OAuth still protects the MCP
...
Filesystem path containment applies to Kastor file tools. Shell commands run
...
By default, Kastor logs requests and tool calls. Shell command previews are
disabled unless `KASTOR_LOG_SHELL_COMMANDS=1`.
